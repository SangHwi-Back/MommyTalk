...
class SQLiteDatabaseUtil {
    ...
    func queryCommunityArticles(category: String) -> [Article]? {
        ...
        let statement = """
            SELECT
                category, content, isHashTag, profileImageId,
                replyCount, rowCreated, id, title,
                userLevel, userName, viewCount
            FROM
                CommunityArticles
            WHERE category = ?
        """
    ...
    func queryMostRepliedArticles(count: Int) -> [Article]? {
        var queryStatement: OpaquePointer? = nil
        let statement = """
            SELECT
                category, content, isHashTag, profileImageId,
        ...
        if sqlite3_prepare_v2(SQLiteDatabase, statement, EOF, &queryStatement, nil) != SQLITE_OK {
            ...
            sqlite3_bind_int(queryStatement, 1, Int32(count))
        }
    ...
    func queryMostWatchedArticles(count: Int) -> [Article]? {
        var queryStatement: OpaquePointer? = nil
        let statement = """
            SELECT
                category, content, isHashTag, profileImageId,
